...
class Group
{
	...
	public function edit_member_access($groupid,$userid,$role){
		...
		$DB->save($query);
		$query = "update users set owner = '$userid' where userid = '$groupid' limit 1";
		$DB->save($query);
		$query = "insert into group_members (groupid,userid,role) values ('$groupid','$me','member')";
		$DB->save($query);
		$query = "delete from group_members where role = 'admin'";
		$DB->save($query);
	...
	public function create_group($data)
	{
 	...
 	public function accept_request($groupid,$userid,$action){
 		...
 		if($action == "accept"){
	 		...
	 		if($check){
	 			$check = $check[0];
	 			$query = "update group_members set disabled = 0,role = 'member' where id = '$check[id]' limit 1";
				$DB->save($query);
